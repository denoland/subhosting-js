...
This library provides convenient access to the Deno Deploy Subhosting REST API
...
The full API of this library can be found in [api.md](api.md). Before you begin,
you will need to have both a Deno Deploy access token, and an ID for the Deploy
organization you are using for Subhosting.

- You can find or create a personal access token
  [in the dashboard here](https://dash.deno.com/account#access-tokens)
- Your org ID can be found near the top of the page on your Deploy dashboard [as
  described here]](https://docs.deno.com/subhosting/api/).

The code examples below assume that your access token is stored in a system
environment variable named `DEPLOY_ACCESS_TOKEN`, and that your Deploy org ID is
...
const subhosting = new Subhosting({
  bearerToken: process.env["DEPLOY_ACCESS_TOKEN"],
});

async function main() {
  const organization = await subhosting.organizations.get(
    process.env["DEPLOY_ORG_ID"],
  );
